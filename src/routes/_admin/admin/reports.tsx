...
function ReportsPage() {
    ...
    try {
      ...
      await logAuditEventFn({
        data: {
          ...
          payload: { reportId: r.id, title: r.title, uid: r.uid },
          userName: "Admin",
        },
      });
  ...
  const handleReprocess = async (r: LegacyReport) => {
    try {
      ...
      setReports((prev) =>
        prev.map((x) => (x.id === r.id ? { ...r, status: "processing" } : x)),
      );
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-mm-gray mt-1">
            View and manage AI-generated reports for all users
          </p>
        </div>
      </div>
              ...
              {isLoading ? (
                ...
                filtered.map((r) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-4 font-extrabold text-xs text-mm-dark max-w-[200px] truncate">
                        {r.title}
                      </td>
                      <td className="px-4 py-4 text-xs font-mono text-mm-gray">
                        {r.uid}
                      </td>
                      ...
                      <td className="px-4 py-4 text-xs text-mm-gray font-semibold">
                        {r.metadata?.tokensUsed?.toLocaleString() ?? "—"}
                      </td>
                      <td className="px-4 py-4 text-xs text-mm-gray font-semibold">
                        {r.metadata?.durationMs
                          ? `${(r.metadata.durationMs / 1000).toFixed(1)}s`
                          : "—"}
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            title="View"
                            className="p-1.5 rounded-lg hover:bg-mm-orange/10 text-mm-orange transition-all cursor-pointer"
                          >
                            <Eye size={15} />
                          </button>
                          {r.status === "failed" && (
                          ...
                          <button
                            ...
                            title="Delete"
                            className="p-1.5 rounded-lg hover:bg-mm-red/10 text-mm-red transition-all cursor-pointer"
                          >
                            <Trash2 size={15} />
                          </button>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl max-h-[85vh] flex flex-col bg-white border border-mm-border rounded-[24px] overflow-hidden shadow-2xl z-50 p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-extrabold text-sm text-mm-dark">
                {viewReport.title}
              </h2>
              <button
                onClick={() => setViewReport(null)}
                className="p-2 rounded-lg hover:bg-mm-subtle text-mm-gray hover:text-mm-dark cursor-pointer transition-colors"
              >
                <X size={18} />
              </button>
            </div>
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <span className="text-mm-gray font-bold">User ID: </span>
                <span className="font-mono font-extrabold text-mm-dark">
                  {viewReport.uid}
                </span>
              </div>
              <div>
                <span className="text-mm-gray font-bold">Status: </span>
                <span
                  className="capitalize font-black"
                  style={{ color: STATUS_COLORS[viewReport.status]?.text }}
                >
                  {viewReport.status}
                </span>
              </div>
              <div>
                <span className="text-mm-gray font-bold">Created: </span>
                <span className="text-mm-dark font-extrabold">
                  {formatDate(viewReport.createdAt)}
                </span>
              </div>
              <div>
                <span className="text-mm-gray font-bold">Tokens: </span>
                <span className="text-mm-dark font-extrabold">
                  {viewReport.metadata?.tokensUsed?.toLocaleString()}
                </span>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-[24px] shadow-2xl p-6 space-y-4 bg-white border border-mm-border z-50">
            ...
            <p className="text-xs text-mm-gray font-medium leading-relaxed">
              ...
              <strong className="text-mm-dark">"{confirmDelete.title}"</strong>.
              This action cannot be undone.
            </p>
            <div className="flex gap-3 justify-end pt-2">
              <button
                onClick={() => setConfirmDelete(null)}
                className="px-4 py-2 rounded-xl text-xs font-bold border border-mm-border text-mm-dark bg-white hover:bg-mm-subtle/50 transition-all cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={() => handleDelete(confirmDelete)}
                className="px-4 py-2 rounded-xl text-xs font-extrabold bg-mm-red text-white hover:bg-mm-red/90 transition-all cursor-pointer"
              >
                Delete
              </button>
            </div>
