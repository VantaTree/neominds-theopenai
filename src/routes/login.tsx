...
import { auth, confirmPasswordResetFn, sendPasswordResetEmailFn } from "@/lib/firebase";
...
function LoginPage() {
  ...
  const handleResetPasswordSubmit = async (e: React.FormEvent) => {
    ...
    if (!search.oobCode) {
      setError("Password reset code (oobCode) is missing.");
      return;
    }
    setIsLoading(true);
    setError("");
    ...
    try {
      await confirmPasswordResetFn(search.oobCode, newPassword);
      setResetSuccess(true);
    } catch (err: any) {
      console.error("Password reset confirmation error:", err);
      setError(err.message || "Failed to confirm password reset. The link may have expired.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleForgotPasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Forgot password link request error:", err);
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-mm-subtle/30 py-12 px-4 font-sans">
      ...
      <div className="w-full max-w-[860px] bg-white rounded-[24px] shadow-2xl shadow-mm-dark/5 flex flex-col md:flex-row overflow-hidden border border-mm-border animate-fadeIn">
        ...
        <div className="w-full md:w-[55%] bg-white p-8 md:p-12 flex flex-col justify-between relative z-20">
          {search.mode === "resetPassword" && search.oobCode ? (
            <div>
              {/* Header */}
              ...
              {resetSuccess ? (
                <div className="space-y-6">
                  <div className="p-4 rounded-xl bg-mm-green/10 border border-mm-green/20 text-mm-green text-sm font-semibold">
                    ✓ Your password has been successfully reset!
                  </div>
                  <Link
                    to="/login"
                    className="w-full py-4 bg-mm-orange text-white font-semibold text-sm rounded-[14px] shadow-lg shadow-mm-orange/20 hover:bg-mm-orange/90 active:scale-[0.98] transition-all mt-4 cursor-pointer flex items-center justify-center"
                  >
                    Back to Login
                  </Link>
                </div>
                ...
                <form onSubmit={handleResetPasswordSubmit} className="space-y-4">
                  ...
                  <div className="text-center mt-4">
                    <Link
                      to="/login"
                      className="text-xs text-mm-orange hover:underline font-semibold"
                    >
                      Back to Login
                    </Link>
                  </div>
            ...
            <div>
              ...
              {resetSuccess ? (
                <div className="space-y-6">
                  <div className="p-4 rounded-xl bg-mm-green/10 border border-mm-green/20 text-mm-green text-sm font-semibold">
                    ✓ Recovery link has been sent to {email}.
                  </div>
                  <button
                    onClick={() => {
                      setView("login");
                ...
                <form onSubmit={handleForgotPasswordSubmit} className="space-y-4">
                  ...
                  <div className="text-center mt-4">
                    <button
                      ...
                      className="text-xs text-mm-orange hover:underline font-semibold cursor-pointer"
                    >
                      Back to Login
                    </button>
                  </div>
                ...
                <div>
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <button
                      ...
                      className="text-xs text-mm-orange hover:underline font-semibold transition-all cursor-pointer focus:outline-none"
                    >
                      Forgot?
                    </button>
                  </div>
