...
import { users as initialUsers, projects as initialProjects } from "./mock-data";
import type { User, Project } from "./mock-data";
import type { Profile, Plan, Subscription, Report, AuditLogEntry, AdminConfig } from "./types";
...
export const getUsers = async (): Promise<User[]> => {
  if (isFirebaseConfigured && db) {
    try {
      ...
      console.error("Firebase getUsers failed, using local storage fallback", e);
    }
  }
...
export const getProjects = async (): Promise<Project[]> => {
  if (isFirebaseConfigured && db) {
    try {
      ...
      console.error("Firebase getProjects failed, using local storage fallback", e);
    }
  }
...
export const getPayments = async (): Promise<any[]> => {
  if (isFirebaseConfigured && db) {
    try {
      ...
      console.error("Firebase getPayments failed, using local storage fallback", e);
    }
  }
...
export const saveProfile = async (uid: string, data: Partial<Profile>): Promise<void> => {
  if (isFirebaseConfigured && db) {
    try {
  ...
  if (idx >= 0) { all[idx] = { ...all[idx], ...data }; } else { all.push({ uid, ...data } as Profile); }
...
const DEFAULT_PLANS: Plan[] = [
  {
    ...
    priceMonthly: 29, priceYearly: 23, billedYearly: 276,
    ...
    maxReports: 5, maxStorageGb: 2, supportLevel: "basic", active: true,
  },
  {
    id: "plus",
    name: "Plus Plan",
    tagline: "Designed for expanding businesses seeking growth.",
    priceMonthly: 59, priceYearly: 47, billedYearly: 564,
    ...
    maxReports: 20, maxStorageGb: 10, supportLevel: "priority", active: true,
  },
  {
    id: "pro",
    name: "Pro Plan",
    tagline: "Ultimate features for scaling market leaders.",
    priceMonthly: 89, priceYearly: 71, billedYearly: 852,
    ...
    maxReports: 100, maxStorageGb: 50, supportLevel: "dedicated", active: true,
  },
];
...
export const getPlans = async (): Promise<Plan[]> => {
  if (isFirebaseConfigured && db) {
    try {
      ...
      if (snap.docs.length > 0) return snap.docs.map(d => ({ id: d.id, ...d.data() }) as Plan);
    } catch (e) {
...
export const deletePlan = async (planId: string): Promise<void> => {
  if (isFirebaseConfigured && db) {
    try { await deleteDoc(doc(db, "plans", planId)); return; } catch (e) { console.error("Firebase deletePlan failed", e); }
  }
  const plans = await getPlans();
  localStorage.setItem(LS_PLANS, JSON.stringify(plans.filter(p => p.id !== planId)));
};
...
export const getSubscriptions = async (): Promise<Subscription[]> => {
  if (isFirebaseConfigured && db) {
    ...
    } catch (e) { console.error("Firebase getSubscriptions failed", e); }
  }
...
export const getUserSubscription = async (uid: string): Promise<Subscription | null> => {
  if (isFirebaseConfigured && db) {
    ...
    } catch (e) { console.error("Firebase getUserSubscription failed", e); }
  }
...
export const saveSubscription = async (sub: Subscription): Promise<void> => {
  ...
  const withId = { ...sub, id };
  if (isFirebaseConfigured && db) {
    try { await setDoc(doc(db, "subscriptions", id), withId); return; } catch (e) { console.error("Firebase saveSubscription failed", e); }
  }
  const all: Subscription[] = JSON.parse(localStorage.getItem(LS_SUBS) || "[]");
  const idx = all.findIndex(s => s.id === id);
  if (idx >= 0) { all[idx] = withId; } else { all.push(withId); }
...
export const getReports = async (): Promise<Report[]> => {
  if (isFirebaseConfigured && db) {
    ...
    } catch (e) { console.error("Firebase getReports failed", e); }
  }
...
export const getReportsByUser = async (uid: string): Promise<Report[]> => {
  if (isFirebaseConfigured && db) {
    try {
      ...
      const snap = await getDocs(q);
      return snap.docs.map(d => ({ id: d.id, ...d.data() }) as Report);
    } catch (e) { console.error("Firebase getReportsByUser failed", e); }
  }
...
export const saveReport = async (report: Report): Promise<void> => {
  ...
  if (isFirebaseConfigured && db) {
    try { await setDoc(doc(db, "reports", id), withId); return; } catch (e) { console.error("Firebase saveReport failed", e); }
  }
  const all: Report[] = JSON.parse(localStorage.getItem(LS_REPORTS) || "[]");
...
export const deleteReport = async (reportId: string): Promise<void> => {
  if (isFirebaseConfigured && db) {
    try { await deleteDoc(doc(db, "reports", reportId)); return; } catch (e) { console.error("Firebase deleteReport failed", e); }
  }
  const all: Report[] = JSON.parse(localStorage.getItem(LS_REPORTS) || "[]");
  localStorage.setItem(LS_REPORTS, JSON.stringify(all.filter(r => r.id !== reportId)));
};
...
export const logAuditEvent = async (uid: string, action: string, payload: Record<string, any>, userName?: string): Promise<void> => {
  const entry: AuditLogEntry = { id: `log_${Date.now()}`, uid, userName, action, payload, timestamp: Date.now() };
  if (isFirebaseConfigured && db) {
    try { await addDoc(collection(db, "auditLog"), entry); return; } catch (e) { console.error("Firebase logAuditEvent failed", e); }
  }
  const all: AuditLogEntry[] = JSON.parse(localStorage.getItem(LS_AUDIT) || "[]");
  all.unshift(entry);
  localStorage.setItem(LS_AUDIT, JSON.stringify(all.slice(0, 500))); // keep last 500
};

export const getAuditLog = async (limitCount = 100): Promise<AuditLogEntry[]> => {
  if (isFirebaseConfigured && db) {
    try {
      ...
      return snap.docs.map(d => ({ id: d.id, ...d.data() }) as AuditLogEntry)
        ...
        .slice(0, limitCount);
    } catch (e) { console.error("Firebase getAuditLog failed", e); }
  }
  const all: AuditLogEntry[] = JSON.parse(localStorage.getItem(LS_AUDIT) || "[]");
...
const DEFAULT_ADMIN_CONFIG: AdminConfig = {
  ...
  welcomeMessage: "Welcome to GrowConsult AI! Your AI-powered business growth platform.",
  updatedAt: Date.now(),
};
...
export const getAdminConfig = async (): Promise<AdminConfig> => {
  if (isFirebaseConfigured && db) {
    ...
    } catch (e) { console.error("Firebase getAdminConfig failed", e); }
  }
  return JSON.parse(localStorage.getItem(LS_ADMIN_CONFIG) || JSON.stringify(DEFAULT_ADMIN_CONFIG));
};

export const saveAdminConfig = async (config: AdminConfig): Promise<void> => {
  const withTs = { ...config, updatedAt: Date.now() };
  if (isFirebaseConfigured && db) {
    try { await setDoc(doc(db, "adminSettings", "config"), withTs); return; } catch (e) { console.error("Firebase saveAdminConfig failed", e); }
  }
